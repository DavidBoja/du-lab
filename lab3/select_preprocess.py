...
input_root = '../datasets/cornell movie-dialogs corpus'
output_root = 'data'
...
def main():
    ...
    selection = os.path.join(output_root, movie_selection)
    selected_movies = read_selected(selection)

    dataset = os.path.join(input_root, full_dataset)
    output = os.path.join(output_root, output_destination)
